...
double  integrator_legend(MathFunc f,double lowerlimit,double upperlimit,unsigned n)
{
	...
	{
		coeff.data[0][i] = 2.0/((1.0-pow(roots.data[0][i],2)) * 
                           pow((value(  derivation_polynomial( polynomial_legende(n),1),roots.data[0][i])),2));
	}
	for(unsigned i = 0 ;i < n;i++)
	{
...
double integrator_laguerre(MathFunc f,double lowerlimit,double upperlimit,unsigned n)
{
	...
	else if ((a == -DBL_MAX)&&( b == DBL_MAX))
	{
		for(unsigned i = 0 ;i < n;i++)
		{
			sum = sum + coeff.data[0][i] * f.eval(roots.data[0][i]) * exp(roots.data[0][i])+coeff.data[0][i]
			            * f.eval(-roots.data[0][i])*exp(roots.data[0][i]);
		}
	}
	else if ((a == -DBL_MAX)&&( b ==-DBL_MAX))
	{
	...
	{
		cout << "The gauss_laguerre method can only make the approximation"
		     << " of this kind of integration:upperlimit == infty or lower limit ==infty" <<endl;
		exit(-1);
	}
	return sum;
}
...
double integrator_Monte_Carlo(MathFunc f,double lowerlimit,double upperlimit,unsigned long n)
{
	...
	{
		cout << "The integrator_Monte_Carlo can only make approximation" 
		     << "for integration from a to b in which a and b isn't infty" << endl;
		exit(-1);
	}
}
